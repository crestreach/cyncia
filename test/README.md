...
- **`sync-all`** (`sync-all.sh` / `sync-all.ps1`): tool scoping, `default_tools` config behavior, `--items` subsets, `--clean` stale output removal, help/error paths, and Codex `AGENTS.override.md` generation / opt-out.
